    ...
    def __getitem__(self, index):
        ...
        if weak_labels != None:
            if isinstance(weak_labels, list):
                ...
                pseudo_labels = weak_labels[1]
                pseudo_labels = [[pseudo_labels[i][0]/duration, pseudo_labels[i][1]/duration] for i in range(len(pseudo_labels))]
            else:
...
def build_collate_data(max_num_frames, max_num_words, frame_dim, word_dim):
    def collate_data(samples):
        ...
        for i, sample in enumerate(samples):
            ...
            if sample['pseudo_labels'] != None:
                pseudo_labels.append(torch.from_numpy(np.array([sample['pseudo_labels']]).astype(np.float32)).squeeze())
        if pseudo_labels != []:
            batch.update({
                'net_input': {
