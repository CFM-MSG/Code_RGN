...
class RGN(nn.Module):
    def __init__(self, config):
        ...
        self.pseudo_trans = DualTransformer(**config['DualTransformer'])
        self.fc_comp = nn.Linear(config['hidden_size'], self.vocab_size)
        self.fc_gauss = nn.Linear(config['hidden_size'], self.num_props*2)
        self.fc_pseudo = nn.Linear(config['hidden_size'], 1)
 
        self.word_pos_encoder = SinusoidalPositionalEmbedding(config['hidden_size'], 0, 20)

    def forward(self, frames_feat, frames_len, words_id, words_feat, words_len, weights, glance=None, **kwargs):
        ...
        if self.training:
            _, p_h = self.pseudo_trans(frames_feat, frames_mask, words_feat + words_pos, words_mask, decoding=1)
            pseudo_width = torch.sigmoid(self.fc_pseudo(p_h[:,-1])).reshape(-1)
            pseudo_center = glance
        ...
        enc_out, h, attn_weight = self.pseudo_trans(props_feat, props_mask, words_feat1, words_mask1, decoding=2, gauss_weight=pos_weight, need_weight=True)
        ...
        if self.use_negative and self.training:
            ...
            _, neg_h_1 = self.pseudo_trans(props_feat, props_mask, words_feat1, words_mask1, decoding=2, gauss_weight=neg_1_weight)
            neg_words_logit_1 = self.fc_comp(neg_h_1)
  
            _, neg_h_2 = self.pseudo_trans(props_feat, props_mask, words_feat1, words_mask1, decoding=2, gauss_weight=neg_2_weight)
            neg_words_logit_2 = self.fc_comp(neg_h_2)

            _, ref_h = self.pseudo_trans(frames_feat, frames_mask, words_feat, words_mask, decoding=2)
            ref_words_logit = self.fc_comp(ref_h)
            pseudo_gauss_weight = self.generate_gauss_weight(props_len, pseudo_center, pseudo_width)
            _, pseudo_ref_h = self.pseudo_trans(frames_feat, frames_mask, words_feat, words_mask, decoding=2, gauss_weight=pseudo_gauss_weight)
            pseudo_ref_words_logit = self.fc_comp(pseudo_ref_h)
        else:
            neg_words_logit_1 = None
            neg_words_logit_2 = None
            ref_words_logit = None
            pseudo_ref_words_logit = None
            pseudo_gauss_weight = None
            pseudo_width = None
        ...
        return {
            ...
            'pseudo_ref_words_logit': pseudo_ref_words_logit,
            'pseudo_gauss_weight': pseudo_gauss_weight,
